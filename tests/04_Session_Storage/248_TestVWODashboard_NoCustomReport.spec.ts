...
test("go directly to dashboard — no login", async ({ page }) => {
    ...
    await expect(page).toHaveURL("https://app.wingify.com/#/dashboard?accountId=1227004");
...
test("go directly to settings — no login", async ({ page }) => {
    ...
    await expect(page).toHaveURL("https://app.wingify.com/#/settings/accounts/general?accountId=1227004");
